...
export default function StaffPage() {
  ...
  const handleSave = () => {
    if (editingStaff) {
      setStaff(prev => prev.map(s => s.id === editingStaff.id ? { ...s, ...form, hourly_rate: form.hourly_rate ? parseFloat(form.hourly_rate) : null, max_daily_bookings: parseInt(form.max_daily_bookings) || 8 } : s));
    } else {
      setStaff(prev => [...prev, { id: `st${Date.now()}`, ...form, hourly_rate: form.hourly_rate ? parseFloat(form.hourly_rate) : null, max_daily_bookings: parseInt(form.max_daily_bookings) || 8, average_rating: 0, total_services_completed: 0, specializations: [] }]);
    }
    setDialogOpen(false);
  };
              ...
              <CardContent className="p-5">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-full bg-muted flex items-center justify-center font-bold">
