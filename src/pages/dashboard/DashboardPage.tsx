...
const statusColors: Record<string, string> = {
  ...
  no_show: 'bg-secondary text-secondary-foreground',
};
...
function BookingRow({ booking }: { booking: any }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 hover:bg-muted/50 transition-colors">
      ...
      <span className={cn("text-[11px] font-medium px-2 py-0.5 rounded-full hidden sm:inline-block", statusColors[booking.status] || 'bg-muted')}>
...
function RequestRow({ request }: { request: any }) {
  return (
    <div className="flex items-center gap-3 px-4 py-3 hover:bg-muted/50 transition-colors">
      ...
      {request.is_urgent && <span className="bg-red-100 text-red-700 text-[10px] px-2 py-0.5 rounded-full font-medium">Urgent</span>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold">Good {getGreeting()} 👋</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Here's what's happening today</p>
        </div>
        <button onClick={() => navigate('/appointments')} className="hidden sm:flex items-center gap-2 px-4 py-2 bg-card border rounded-lg text-sm font-medium hover:bg-muted/50 transition-colors">
          <Calendar className="w-4 h-4" /> View Calendar
        </button>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <MetricCard icon={DollarSign} label="Today's Revenue" value={`$${todayRevenue.toFixed(2)}`} accent="bg-emerald-100 text-emerald-600" />
        ...
        <MetricCard icon={Clock} label="Pending" value={pendingRequests.length} sub="awaiting response" accent="bg-violet-100 text-violet-600" />
      </div>
      ...
      <div className="grid lg:grid-cols-5 gap-4">
        <div className="lg:col-span-3 bg-card rounded-lg border">
          <div className="flex items-center justify-between px-4 py-3 border-b">
            ...
            <span className="text-xs font-medium text-muted-foreground bg-muted px-2 py-0.5 rounded-full">{todayBookings.length} bookings</span>
          </div>
      ...
      <div className="bg-card rounded-lg border">
        <div className="px-4 py-3 border-b"><h2 className="text-sm font-semibold">This Week's Revenue</h2></div>
        <div className="p-4">
