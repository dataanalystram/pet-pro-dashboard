...
import { mockMessages, mockThread } from '@/lib/mock-data';
...
export default function MessagesPage() {
  ...
  const sendMsg = (e: React.FormEvent) => {
    ...
    setThread(prev => [...prev, { id: `m${Date.now()}`, content: newMessage.trim(), sender: 'provider', created_at: new Date().toISOString() }]);
    setNewMessage('');
  };
        ...
        <Card className={cn("lg:col-span-1", activeConvo && "hidden lg:block")}>
          <CardContent className="p-0 h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto divide-y">
              {filtered.map((convo) => (
                <button key={convo.customer_email} onClick={() => openThread(convo)}
          ...
          {!activeConvo ? (
            ...
            <CardContent className="p-0 h-full flex flex-col">
              <div className="flex items-center gap-3 px-4 py-3 border-b">
                <Button variant="ghost" size="icon" className="lg:hidden" onClick={() => setActiveConvo(null)}>
                  <ArrowLeft className="w-4 h-4" />
                </Button>
                <div className="w-8 h-8 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-semibold text-sm">
                  {activeConvo.customer_name[0]}
                </div>
